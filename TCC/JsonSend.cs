...
namespace TCC
{
    class JsonSend<I, O>
    {
        public async Task<O> DoPostRequest(string url, I input)
        {
            ...
            var uri = new Uri(string.Format(url,"action","post", DateTime.Now.Ticks));
            ...
            {
                MessageDialog msgbox = new MessageDialog("While puting: " + url + " we got the following status code: " + x.StatusCode);
            }
